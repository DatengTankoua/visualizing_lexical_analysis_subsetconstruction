...
export function convertNFAtoDFAWithSteps(nfa: NFA): SubsetConstructionResult {
  ...
  steps.push({
    ...
    description: `Initialization: ε-closure({${nfa.startState}}) = {${startState.join(',')}}`,
  ...
  while (unmarkedStates.length > 0) {
    ...
    steps.push({
      stepNumber: stepNumber++,
      description: `Select unmarked state: ${stateSetToString(currentState)}`,
    ...
    for (const symbol of alphabet) {
      ...
      if (newState.length === 0) {
        ...
        steps.push({
          stepNumber: stepNumber++,
          description: `ε-closure({${reachableStates.join(',') || '∅'}}) = ∅ → No new transition`,
          currentState: currentState,
          currentStateString: stateSetToString(currentState),
          symbol: symbol,
          moveResult: reachableStates,
      ...
      if (isNewState) {
        ...
        steps.push({
          stepNumber: stepNumber++,
          description: `New DFA state found: ${stateSetToString(newState)}`,
          currentState: currentState,
          currentStateString: stateSetToString(currentState),
          symbol: symbol,
          epsilonClosureResult: newState,
        ...
        steps.push({
          stepNumber: stepNumber++,
          description: `State ${stateSetToString(newState)} already exists`,
          currentState: currentState,
          currentStateString: stateSetToString(currentState),
          symbol: symbol,
          epsilonClosureResult: newState,
      ...
      steps.push({
        stepNumber: stepNumber++,
        description: `Transition added: ${stateSetToString(currentState)} -${symbol}→ ${stateSetToString(targetState)}`,
        currentState: currentState,
        currentStateString: stateSetToString(currentState),
        symbol: symbol,
        isNewState: false,
  ...
  steps.push({
    stepNumber: stepNumber++,
    description: `Done! DFA constructed with ${dfa.states.length} states and ${dfa.transitions.length} transitions`,
    currentState: [],
    currentStateString: '',
    isNewState: false,
    dfaStates: JSON.parse(JSON.stringify(dfaStates)),
